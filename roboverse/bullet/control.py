...
def apply_action_ik(target_ee_pos, target_ee_quat, target_gripper_state,
    ...
    bodyInfo = p.getBodyInfo(robot_id)
    if bodyInfo[1].decode("ascii") == 'ur5robot_gripper':
        apply_action_ik_ur5(target_ee_pos, target_ee_quat, target_gripper_state,
                    robot_id, end_effector_index, movable_joints,
                    lower_limit, upper_limit, rest_pose, joint_range,
                    num_sim_steps)
    elif bodyInfo[1].decode("ascii") == 'wx250s':
        apply_action_ik_widow250(target_ee_pos, target_ee_quat, target_gripper_state,
                    robot_id, end_effector_index, movable_joints,
                    lower_limit, upper_limit, rest_pose, joint_range,
                    num_sim_steps)
    else:
        print("No robot found, bodyInfo:", bodyInfo)
...
def apply_action_ik_ur5(target_ee_pos, target_ee_quat, target_gripper_poses,
    ...
    target_joint_poses = list(joint_poses[0:6]) + target_gripper_poses
    
    max_forces=[500]*len(target_joint_poses)
    # pos_gains = [0.08]*(len(target_joint_poses)-len(target_gripper_poses))+[0.08]*len(target_gripper_poses)
    pos_gains = [0.08]*len(target_joint_poses)
...
# Orginal Function
def apply_action_ik_widow250(target_ee_pos, target_ee_quat, target_gripper_state,
                    robot_id, end_effector_index, movable_joints,
                    lower_limit, upper_limit, rest_pose, joint_range,
                    num_sim_steps=5):
    joint_poses = p.calculateInverseKinematics(robot_id,
