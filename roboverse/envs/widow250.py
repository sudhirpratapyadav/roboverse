...
class Widow250Env(gym.Env, Serializable):
    ...
    def __init__(self,
                 ...
                 ee_pos_high=(1.0, 1.0, -0.1),
                 ee_pos_low=(0.0, -0.1, -.34),
        ...
        if self.load_objects:
            ...
            self.points = [
                    [self.object_position_low[0], self.object_position_low[1], self.object_position_low[2]-0.07],
                    [self.object_position_high[0], self.object_position_low[1], self.object_position_low[2]-0.07],
                    [self.object_position_high[0], self.object_position_high[1], self.object_position_low[2]-0.07],
                    [self.object_position_low[0], self.object_position_high[1], self.object_position_low[2]-0.07],
                ]
    ...
    def get_observation(self):
        ...
        if self.observation_mode == 'pixels':
            ...
            if type(self).__name__=='Widow250DrawerRandomizedEnv':
                observation = {
                    'object_position': np.array(self.drawer_pos),
                    'object_orientation': np.array(self.drawer_quat),
                    'state': np.concatenate(
                        (ee_pos, ee_quat, gripper_state, gripper_binary_state)),
                    'image': image_observation
                }
            elif type(self).__name__=='Widow250ButtonEnv':
                observation = {
                    'object_position': np.array(self.button_pos),
                    'object_orientation': np.array(self.button_quat),
                    'state': np.concatenate(
                        (ee_pos, ee_quat, gripper_state, gripper_binary_state)),
                    'image': image_observation
                }
